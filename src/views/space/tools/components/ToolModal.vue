...
interface Props {
  visible: boolean
  provider?: GetAPIToolProvidersWithPage
}
  ...
  if (formData.openapi_schema)
    if (formData.openapi_schema.trim()) {
      ...
      try {
        ...
        // 无论成功失败，都重置加载状态
        initialToolProvider.openapi_schema = formData.openapi_schema
...
const handleSubmit = async ({
  ...
  // 如果数据没有更改，直接返回
  if (deepEqual(values, initialToolProvider)) {
    Message.warning('请编辑插件信息')
    return
  }
...
const handleDeleteTool = async () => {
  ...
  Modal.warning({
    ...
    onOk: async () => {
      ...
      try {
        // 调用API删除工具提供者
        const resp = await APIToolsApi.deleteAPIToolProvider(props.provider.id)
        // 显示删除成功的消息
...
const updateFormData = (data: {
  ...
  // 更新初始工具提供者数据
  Object.assign(initialToolProvider, data)
}
          ...
          <a-form-item label="Headers">
            ...
            >
              ...
              <template #value="{ rowIndex }">
                <a-form-item hide-label class="m-0">
                  <a-input v-model="formData.headers[rowIndex].value" placeholder="请输入键值" />
                </a-form-item>
              </template>
