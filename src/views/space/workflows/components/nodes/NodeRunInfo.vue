...
const handleClick = () => {
  const dataId = props.data.node_results[0].node_data.id
    ...
    >
      <a-collapse-item :key="data?.id">
          ...
          <div class="flex items-center justify-between w-full" @click.stop>
            <div class="flex items-center gap-1.5">
              ...
              <a-tag class="rounded-md text-[#00b23c]" size="small" color="#69d18c4d"
                >{{ (data.node_results[0].latency as number)?.toFixed(3) }}s</a-tag
              >
            </div>
          </div>
        </template>
        ...
        <div v-if="!isHideInput" class="flex flex-col gap-1" @click.stop>
          <div class="flex items-center gap-0.5">
            ...
            <a-tooltip content="复制输入值">
              <a-button
                ...
                size="mini"
                class=""
                @click="copyToClipboard(data.node_results[0].inputs)"
              >
          ...
          <div
            v-if="!isEmpty(data.node_results[0].inputs)"
            class="border border-gray-200 rounded-md"
          >
            <vue-json-pretty
              :data="data.node_results[0].inputs"
              show-length
              :deep="1"
              :show-line="false"
        ...
        <div v-if="!isHideOutput" class="flex flex-col gap-1" @click.stop>
          ...
          <div
            v-if="!isEmpty(data.node_results[0].outputs)"
            class="border border-gray-200 rounded-md"
          >
            <vue-json-pretty
              :data="data.node_results[0].outputs"
              show-length
              :deep="1"
              :show-line="false"
              class="overflow-y-scroll scrollbar-w-none flex-1 p-3 text-xs"
