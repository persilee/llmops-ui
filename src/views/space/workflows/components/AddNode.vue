...
<template>
  <div
    class="flex flex-col gap-2 px-2 py-3 bg-white rounded-md shadow-sm border border-gray-200 w-[320px]"
  >
    <div class="flex gap-2">
      <!-- 开始节点 -->
      <a-tooltip content="工作流的起始节点，支持定义工作流的起点输入等信息。" position="left">
        <div
      ...
      <!-- 结束节点 -->
      <a-tooltip content="工作流的结束节点，支持定义工作流最终输出的变量等信息。" position="right">
        <div
          class="flex-1 flex items-center gap-2 p-2 cursor-pointer hover:bg-gray-100 rounded-md"
          @click="() => addNode('end')"
        >
    ...
    <div class="flex gap-2">
      <!-- 大语言模型节点 -->
      <a-tooltip content="调用大语言模型，根据输入参数和提示词生成回复。" position="left">
        <div
          class="flex-1 flex items-center gap-2 p-2 cursor-pointer hover:bg-gray-100 rounded-md"
          @click="() => addNode('llm')"
        >
      ...
      <a-tooltip content="添加插件广场内或自定义API插件，支持能力扩展和复用。" position="right">
        <!-- 扩展插件 -->
        <div
          class="flex-1 flex items-center gap-2 p-2 cursor-pointer hover:bg-gray-100 rounded-md"
          @click="() => addNode('tool')"
    ...
    <div class="flex gap-2">
      <!-- 知识库检索 -->
      <a-tooltip
        content="根据输入的参数，在选定的知识库中检索相关片段并召回，返回切片列表。"
        position="left"
      >
        <div
          class="flex-1 flex items-center gap-2 p-2 cursor-pointer hover:bg-gray-100 rounded-md"
      ...
      <a-tooltip content="对多个字符串变量的格式进行处理。" position="right">
        <!-- 模板转换 -->
        <div
          class="flex-1 flex items-center gap-2 p-2 cursor-pointer hover:bg-gray-100 rounded-md"
    ...
    <div class="flex gap-2">
      <!-- 意图识别节点 -->
      <a-tooltip
        content="定义用户问题的分类条件，LLM能够根据分类描述执行不同的分支。"
        position="left"
      >
        <div
          class="flex-1 flex items-center gap-2 p-2 cursor-pointer hover:bg-gray-100 rounded-md"
      ...
      <a-tooltip
        content="传递一个列表型数据，并引用一个工作流完成多轮迭代得到处理后的列表数据。"
        position="right"
      >
        <!-- 迭代节点 -->
        <div
    ...
    <div class="flex gap-2">
      <!-- 节点名称 -->
      <a-tooltip content="配置外部API服务，并发起请求。" position="left">
        <div
          class="flex-1 flex items-center gap-2 p-2 cursor-pointer hover:bg-gray-100 rounded-md"
      ...
      <a-tooltip content="编写代码处理输入输出变量来生成返回值。" position="right">
        <!-- 代码执行 -->
        <div
          class="flex-1 flex items-center gap-2 p-2 cursor-pointer hover:bg-gray-100 rounded-md"
          @click="() => addNode('code')"
