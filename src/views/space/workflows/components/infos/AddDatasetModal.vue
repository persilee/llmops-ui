...
const props = defineProps<{
  nodeDatasets: Dataset[]
}>()
const emits = defineEmits(['updateNodeDatasets'])
// 控制抽屉组件的显示/隐藏状态
...
const isAdded = (dataset: GetDatasetsWithPage) => {
  return selectedDatasets.value.some((draftDataset: Dataset) => draftDataset.id == dataset.id)
}
...
const handleClick = (dataset: GetDatasetsWithPage) => {
  ...
  if (isAdded(dataset)) {
    // 如果已添加，则从列表中移除该知识库
    _.remove(selectedDatasets.value, (draftDataset: Dataset) => draftDataset.id == dataset.id)
  } else {
    if (selectedDatasets.value.length >= 5) {
      Message.warning('关联知识库已超过5个，无法继续关联')
      return
    }
    // 如果未添加，则将知识库添加到列表中
    selectedDatasets.value.push({
...
const handleSave = async () => {
  emits('updateNodeDatasets', selectedDatasets.value)
  handleClose()
}
...
const stop = watch(visible, async (val) => {
  if (val) {
    ...
    selectedDatasets.value = _.cloneDeep(props.nodeDatasets)
  }
})
      ...
      <div class="flex flex-col bg-white w-full h-full relative">
        ...
        >
          <div
            ...
            :key="dataset.id"
            :class="`${isAdded(dataset) ? 'bg-blue-50 border border-blue-600' : 'border border-gray-200'} flex items-center gap-2  p-2 rounded-lg cursor-pointer hover:bg-blue-50`"
